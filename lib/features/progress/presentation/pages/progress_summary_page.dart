...
class _ProgressSummaryPageState extends State<ProgressSummaryPage> {
  ...
  List<SessionLog> _recentSessions(List<SessionLog> sessions) {
  ...
  ProgressStats _statsFor(List<SessionLog> sessions) => switch (_range) {
    ProgressRange.all => computeAllTimeProgressStats(sessions),
    ProgressRange.monthly => computeMonthlyProgressStats(sessions),
    ProgressRange.weekly => computeWeeklyProgressStats(sessions),
  };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<RoutineItemsCubit, RoutineItemsState>(
        builder: (context, state) {
          return switch (state) {
            RoutineItemsLoaded(:final items) => _buildBody(context, items),
            RoutineItemsFailureState(:final message) => _ProgressError(
              message: message,
              onRetry: () => context.read<RoutineItemsCubit>().fetch(),
            ),
  ...
  Widget _buildBody(BuildContext context, List<RoutineItem> routineItems) {
    final sessions = buildSessionLogs(routineItems);
    final stats = _statsFor(sessions);
    final recentSessions = _recentSessions(sessions);

    return RefreshIndicator(
      onRefresh: () => context.read<RoutineItemsCubit>().fetch(),
          ...
          child: Column(
            ...
            children: [
              ...
              Center(
                child: TextButton(
                  onPressed: () =>
                      context.push('/progress/history', extra: routineItems),
                  child: Row(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            MainButton(label: 'Coba Lagi', onPressed: onRetry),
          ],
        ),
      ),
    );
